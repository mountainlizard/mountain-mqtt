# `embassy-poll-example`

A simple MQTT example using the `PollClient` approach, to run on either a [Raspberry Pi Pico](https://www.raspberrypi.com/products/raspberry-pi-pico/) or [Raspberry Pi Pico 2](https://www.raspberrypi.com/products/raspberry-pi-pico-2/).

## Running the Example

You will need the wireless-equipped version of a Raspberry Pi Pico or Pico 2 board.

Note that this crate contains the shared code suitable for running on either Pico or Pico 2 - the actual binary projects are in `embassy-poll-example-rp2040w` for the Pico, and `embassy-poll-example-rp2040w` for the Pico 2.

Before running the example, make sure your Pico board is connected to power, and connect a [Raspberry Pi Debug Probe](https://www.raspberrypi.com/documentation/microcontrollers/debug-probe.html) to the Pico debug port and via USB to your development PC. You may also need to [install `probe-rs`](https://probe.rs/docs/getting-started/installation/). A debug probe is strongly recommended, but you can also [try this approach to run without one](https://embassy.dev/book/#_how_to_deploy_to_rp2040_or_rp235x_without_a_debugging_probe).
...
You will need to specify the WiFi SSID/password and MQTT server host/port using environment variables when building and flashing the examples. First, change to the `embassy-poll-example-rp2040w` for the Pico, and `embassy-poll-example-rp2040w` for the Pico 2. Then run the following, making sure to adapt each environment variable above to your WiFi and MQTT server:
...
You will see log lines showing the WiFi connection starting up. When this is complete, check for activity on your server (for example using [MQTT Explorer](https://mqtt-explorer.com)). You should see a "presence" message:

```text
embassy-poll-example-presence = true
```

If you have connected a button, you can press and release it to see messages on `embassy-poll-example-button` - this should be `true` when the button is held down, and `false` when it is released.

Finally, you can send a message to `embassy-poll-example-led` with `true` to turn on the on-board LED on the Pico board, and `false` to turn it off.
...
- the WiFi `stack` for communications,
- `uid_handle` with the unique id produced from the microcontroller's embedded id (we'll use this to produce the MQTT client id),
...
The `client_function` is just an async fn that receives a `PollClient`. In the example we've defined a type alias `OurPollClient` just to avoid repeating the parameters multiple times. We use the `PollClient` to talk to the MQTT server - we can do what we want with this client, but typically we will:
...
3. Receive incoming application messages that are published to topics we have subscribed to
4. Publish application messages to topics on the server.
...
In `mqtt::run`, the `client_function` is a closure, which calls through to `client_function_with_channels`, passing the client, UID and our channel publisher and subscriber. The code here should be fairly self-explanatory, but there are a few points to note.
...
Looking through the rest of the code in `mqtt_poll` we can see that it's very similar to the `mqtt` code. We just need to call `wait_for_responses` after each `PollClient` method that sends packets (this is noted in the docs for the relevant client methods). The final loop is also a little different since it receives a `PacketBin` and then processes it, rather than just a plain call to `client.receive()`.

In general, if you can use `HandlerClient` it should be much simpler. However, if it's difficult to produce a `SyncEventHandler` that works for the whole lifetime of an MQTT connection then `PollClient` allows you to adopt a more manual approach to handling incoming messages. It's worth noting that `HandlerClient` is just a simple wrapper around `PollClient` that uses a very similar approach to that shown in `mqtt_poll` to wait for responses and pass them to the handler.
