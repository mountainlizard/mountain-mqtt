...
pub async fn demo_poll_result(
    ...
    loop {
        match select(action_sub.next_message_pure(), client.receive()).await {
            Either::First(action) => {
                send_action(&mut client, &action).await?;
                wait_for_responses(&mut client, event_pub).await?;
            }
            Either::Second(packet_bin) => {
                let packet_bin = packet_bin?;
                receive_event(&mut client, packet_bin, event_pub).await?;
            }
        }
    }
}
...
pub async fn run(
    ...
    loop {
        info!("run: Trying MQTT connection, will use polling");
        ...
        let poll = async |client: Client<'_>| {
