...
mod tests {
    ...
    fn encode_decode_and_check<const P: usize>(packet: &Pubrel<'_, P>, encoded: &[u8]) {
        encode_and_check(packet, encoded);
        decode_and_check(packet, encoded);
    }

    fn encode_and_check<const P: usize>(packet: &Pubrel<'_, P>, encoded: &[u8]) {
    ...
    fn decode_and_check<const P: usize>(packet: &Pubrel<'_, P>, encoded: &[u8]) {
