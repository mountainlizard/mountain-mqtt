...
async fn main(spawner: Spawner) {
    ...
    // Create uid handle with a random number.
    core::write!(uid_handle, "embassy-mqtt-{}",  rng.next_u64()).unwrap();
    ...
    let pin12 =  Input::new(p.PIN_12, gpio::Pull::Down);

    unwrap!(spawner.spawn(ui_task(event_sub_ui, action_pub_ui, pin12, control)));
