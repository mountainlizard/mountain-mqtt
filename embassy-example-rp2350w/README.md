...
A simple MQTT example to run on an Raspberry Pi Pico W (or other compatible RP2350 board with wireless)
...
You will need to specify the WiFi SSID/password and MQTT server host/port using environment variables when building and flashing the example. Run the following from the `embassy-example-rp2350w` directory, making sure to adapt each environment variable above to your WiFi and MQTT server:
...
❯ MQTT_HOST='192.168.0.201' MQTT_PORT='1883' WIFI_NETWORK='mint' WIFI_PASSWORD='PXgu17wAIcPM' cargo run --release
   Compiling cyw43-pio v0.8.0
...
    Finished `release` profile [optimized + debuginfo] target(s) in 36.32s
     Running `probe-rs run --chip RP235x --protocol swd target/thumbv8m.main-none-eabihf/release/embassy-example-rp2350w`
      Erasing ✔ 100% [####################] 372.00 KiB @  77.81 KiB/s (took 5s)
  Programming ✔ 100% [####################] 372.00 KiB @  34.39 KiB/s (took 11s)                                                           Finished in 15.60s
0.000748 [INFO ] Hello World! (embassy_example_rp2350w embassy-example-rp2350w/src/main.rs:65)
0.479285 [WARN ] BDC event, incomplete header (cyw43 cyw43-0.5.0/src/runner.rs:453)
...
```
...
```text
embassy-example-rp2350w-presence = true
```

If you have connected a button, you can press and release it to see messages on `embassy-example-rp2350w-button` - this should be `true` when the button is held down, and `false` when it is released.

Finally, you can send a message to `embassy-example-rp2350w-led` with `true` to turn on the on-board LED on the Pico board, and `false` to turn it off.
...
In the case of `Action`, we handle the `Button` variant by sending a message to the topic `"embassy-example-rp2350w-button"` containing the `bool` value encoded as JSON.
